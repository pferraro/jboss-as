...
public class DistributedSingletonServiceBuilder<T> implements SingletonServiceBuilder<T>, Service<T>, ServiceProviderRegistration.Listener, SingletonContext<T>, Singleton {
    ...
    public Set<Node> getProviders() {
        return this.registration.getValue().getProviders();
    }

    @Override
    public Optional<Node> getPrimaryProvider() {
        if (this.isPrimary()) {
            return Optional.of(this.registry.getValue().getGroup().getLocalNode());
        }
        try {
            Map<Node, CommandResponse<Boolean>> responses = this.dispatcher.getValue().executeOnCluster(new IsPrimaryCommand<>());
