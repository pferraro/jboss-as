...
public class BinderServiceBuilder<T> implements CapabilityServiceBuilder<ManagedReferenceFactory> {
    ...
    public ServiceBuilder<ManagedReferenceFactory> build(ServiceTarget target) {
        if (!this.enabled) {
            // If naming is not enabled, just install a dummy service
            return target.addService(this.getServiceName(), new ValueService<>(new ImmediateValue<ManagedReferenceFactory>(null))).setInitialMode(ServiceController.Mode.NEVER);
        }
