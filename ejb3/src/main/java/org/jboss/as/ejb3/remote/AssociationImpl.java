...
final class AssociationImpl implements Association, AutoCloseable {
    ...
    AssociationImpl(final DeploymentRepository deploymentRepository, final List<Map.Entry<RemotingConnectorInfo, Registry<String, List<ClientMapping>>>> clientMappingRegistries) {
        ...
        for (Map.Entry<RemotingConnectorInfo, Registry<String, List<ClientMapping>>> entry : clientMappingRegistries) {
            this.clusterTopologyRegistrars.put(entry.getKey().getSocketBinding().getPort(), new ClusterTopologyRegistrar(entry.getValue()));
        }
    }
    ...
    public ListenerHandle registerClusterTopologyListener(@NotNull final ClusterTopologyListener clusterTopologyListener) {
        // TODO Only register topology listener with a specific ClusterTopologyRegistrar
        List<ListenerHandle> handles = this.clusterTopologyRegistrars.isEmpty() ? Collections.emptyList() : new ArrayList<>(this.clusterTopologyRegistrars.size());
        for (ClusterTopologyRegistrar registrar : this.clusterTopologyRegistrars.values()) {
            handles.add(registrar.registerClusterTopologyListener(clusterTopologyListener));
        }
    ...
    private Affinity getStatelessAffinity(Request request) {
        SocketAddress localAddress = request.getLocalAddress();
        ClusterTopologyRegistrar registrar = (localAddress instanceof InetSocketAddress) ? this.clusterTopologyRegistrars.get(((InetSocketAddress) localAddress).getPort()) : null;
